...
placeholder = ParagraphStyle("ph", parent=styles["Normal"],
...
story.append(Paragraph("C<sub>3D,max</sub> = 1 / sin²(θ<sub>s</sub>) ≈ 46,200", ParagraphStyle(
    "f", parent=styles["Normal"], fontSize=10, leftIndent=20,
    fontName="Helvetica-Oblique", textColor=colors.HexColor("#333366"), spaceAfter=5)))
...
story.append(Paragraph("C<sub>2D,max</sub> = 1 / sin(θ<sub>s</sub>) ≈ 215", ParagraphStyle(
    "f2", parent=styles["Normal"], fontSize=10, leftIndent=20,
    fontName="Helvetica-Oblique", textColor=colors.HexColor("#333366"), spaceAfter=5)))
...
systems_2d = [
    ("a) Colector de Canal Parabólico (CCP)",
     "Espejo con perfil parabólico que concentra la luz en un tubo receptor a lo largo de la línea focal. "
     "Es el más utilizado en plantas termosolares comerciales.",
     "Buscar: \"parabolic trough collector solar\""),
    ("b) Concentrador de Fresnel Lineal (LFC)",
     "Filas de espejos planos o curvados que reflejan la luz hacia un receptor fijo ubicado arriba. "
     "Es más barato que el CCP aunque con menor eficiencia óptica.",
     "Buscar: \"linear Fresnel reflector solar\""),
    ("c) Concentrador Parabólico Compuesto — CPC (Winston)",
     "Formado por dos secciones parabólicas que dirigen toda la luz del ángulo de aceptancia "
     "hacia el receptor. Puede funcionar sin seguimiento o con seguimiento mínimo.",
     "Buscar: \"compound parabolic concentrator CPC solar\""),
]

for name, desc, hint in systems_2d:
    story.append(Paragraph(f"<b>{name}:</b> {desc}", resp))
    story.append(Paragraph(hint, placeholder))
    # Recuadro para foto
    table = Table([["Insertar fotografía aquí"]], colWidths=[13*cm], rowHeights=[3.5*cm])
...
systems_3d = [
    ("a) Disco Parabólico",
     "Espejo en forma de paraboloide de revolución que concentra la luz en un punto focal. "
     "Alcanza los mayores factores C (hasta ~3,000) y generalmente va acoplado a un motor Stirling.",
     "Buscar: \"parabolic dish solar concentrator Stirling\""),
    ("b) Torre Solar con Heliostatos",
     "Campo de espejos planos (heliostatos) que reflejan individualmente la luz hacia un receptor "
     "en lo alto de una torre central. Factor de concentración típico entre 500 y 1,000.",
     "Buscar: \"solar power tower heliostat field\""),
    ("c) Lente de Fresnel Circular",
     "Lente plana escalonada que aproxima una lente convexa con menor grosor y peso. Concentra "
     "la luz en un punto focal y se usa en sistemas fotovoltaicos de concentración (CPV).",
     "Buscar: \"circular Fresnel lens solar CPV\""),
]

for name, desc, hint in systems_3d:
    story.append(Paragraph(f"<b>{name}:</b> {desc}", resp))
    story.append(Paragraph(hint, placeholder))
    table = Table([["Insertar fotografía aquí"]], colWidths=[13*cm], rowHeights=[3.5*cm])
    table.setStyle(TableStyle([
...
story.append(Paragraph(
    ...
    "una versión 3D en forma de trompeta, que alcanza C<sub>3D,max</sub> = 1/sin²(θ<sub>a</sub>).",
    resp))
